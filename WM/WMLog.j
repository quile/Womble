...
*/
var MESSAGE_BUFFER = [WMArray new];
var START_TIME = 0;


@implementation WMLog : WMObject
...
+ endLoggingTransaction:(WMRequest)r {
    ...
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_INFO addMessage:"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"]];
}
...
+ formatAsHtml:(id)dumpMask {
    ...
    var messageTable = "";
    messageTable = messageTable + [WMLog htmlOpenTable:dumpMask];
    messageTable = messageTable + [WMLog htmlColumnHeaders:dumpMask];

    for (var i=0; i< [MESSAGE_BUFFER count]; i++) {
        var message = [MESSAGE_BUFFER objectAtIndex:i];
        if (!(dumpMask & [message type])) { continue; }
        messageTable = messageTable + [WMLog htmlRowFromLogEvent:message dumpMask:dumpMask];
    }
    messageTable = messageTable + [WMLog htmlCloseTable:dumpMask];
    return messageTable;
}

+ dumpLogForRequest:(id)request usingLogMask:(id)logMask {
    var messageTable = [WMLog formatAsHtml:logMask];
    [request print:messageTable];
    messageTable = null;
}

+ debug:(id)msg {
    var logMessage = [WMLogMessage newWithType:CODE_DEBUG message:msg depth:_pageStructureDepth];
    [WMLog addMessage:logMessage];
}
...
+ page:(id)message {
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_STRUCTURE message:message depth:_pageStructureDepth]];
}

+ info:(id)message {
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_INFO message:message depth:_pageStructureDepth]];
}

+ database:(id)message {
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_DATABASE message:message depth:_pageStructureDepth]];
}

+ error:(id)message {
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_ERROR message:message depth:_pageStructureDepth]];
    //throw [CPException raise:"CPException" reason:message];
}

+ assert:(id)assertion message:(id)message {
    if (!assertion) {
        var logMessage = [WMLogMessage newWithType:CODE_ASSERTION message:"ASSERTION FAILED: " + message depth:_pageStructureDepth];
        [WMLog addMessage:logMessage];
    }
    return assertion;
}
...
+ warning:(id)message {
    [WMLog addMessage:
        [WMLogMessage newWithType:CODE_WARNING message:message depth:_pageStructureDepth]];
}

+ code:(id)message {
    [WMLog addMessage:
        [WMLogMessage newtWithType:CODE_CODE message:message depth:_pageStructureDepth]];
}
...
+ logQueryDictionaryFromContext:(id)context {
    ...
    foreach var key (keys %queryDictionary) {
        ...
        var logMessage = [WMLogMessage new:"key = value" :_pageStructureDepth](CODE_QUERY_DICTIONARY, "key = value", _pageStructureDepth);
