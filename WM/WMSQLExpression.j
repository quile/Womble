...
@implementation WMSQLExpression : WMObject
{
}
...
- derivedDataSourceWithName:(id)name {
    ...
    if (t && [self isDerivedTable:t]) {
        return [WMRelationshipDerived newFromFetchSpecification:t.DEFINITION withName:name];
    }
    return nil;
}
...
- (void) addTraversedRelationship:(id)relationshipName onEntity:(id)entityClassDescription :(id)shouldTraverseToRepeatedTable {
    ...
    if (shouldTraverseToRepeatedTable) {
        [WMLog debug:"Traversing to repeated table via " + relationshipName + " on " + [entityClassDescription name]]
    }
    ...
    if (shouldTraverseToRepeatedTable) {
        ...
        if ([WMLog assert:[self tableWithName:newTableName] message:"newTableName is already registered"]) {
...
- addDynamicRelationship:(id)dr {
    if (![WMLog assert:dr message:"Adding dynamic relationship"]) { return }
...
- sortOrderingsAsSQL {
    ...
    for (var i=0; i < self._sortOrderings.length; i++) {
        ...
        if (orderGoo) {
            ...
            [WMLog error:"Couldn't figure out what " + o + " means for ordering"];
        }
        if (orderTerm) {
...
/* This is also a rehash of the same code that's used in Qualifier to generate
   SQL... and also in WM::SummaryAttribute to generate its SQL too.  We can almost
...
- groupByAsSQL {
    ...
    var defaultTableAlias = [self aliasForTable:self._defaultTable];
    var defaultEntityClass = [self entityClassDescriptionForTableWithName:self._defaultTable];
    //WM::Log::debug("doing groupings for table $self[_defaultTable}...");
    foreach var grouping (@{self._groupBy}) {
        //WM::Log::debug("Checking for grouping $grouping");
        var tableAlias;
        var columnName;
        var groupBy;
        if (defaultEntityClass) {
            ...
            if (attributeName =~ /\ + /) {
                ...
                if (self._traversedRelationships[relationshipName} || [self dynamicRelationshipWithName:relationshipName]) {
                    ...
                    if (relationship) {
                        ...
                        if (WMLog.assert(targetEntityClassDescription, "Located target entity class description for relationshipName")) {
                            ...
                            tableAlias = [self aliasForTable:targetEntityClassDescription->_table()];
                            columnName = [targetEntityClassDescription columnNameForAttributeName:attributeName];
                            if (WMLog.assert(tableAlias && columnName, "We have a target table and column")) {
                                groupBy = "tableAlias + columnName";
                            }
                        }
                    } else {
                        WMLog.error("Didn't find relationship relationshipName");
                    }
                } else {
                    // maybe it's an explicit table name or table alias:
                    //  WM::Log::debug("Checking for table or alias name for $relationshipName");
                    ...
                    //WM::Log::debug("....... grouping by $groupBy");
                }
            } else {
                //WM::Log::debug("Grouping by a straight attribute");
                tableAlias = defaultTableAlias;
...
- dictionaryOfEntitiesFromRawRow:(id)row {
    ...
    for (var ki = 0; ki < [keys count]; ki++) {
        ...
        if (!r) { [WMLog error:"Didn't match regexp for key " + key]; continue;  }
        var tableAlias = r[1];
        var columnName = r[2];
        var tableName = [self tableNameForAlias:tableAlias];
        tables[tableName] = tables[tableName] || [WMDictionary new];
...
- dictionaryFromRawRow:(id)row {
    var dictionary = [WMDictionary new];
    ...
    for (var i=0; i < [allKeys count]; i++) {
        ...
        [WMLog debug:"setting " + mappedKey + " to " + [row objectForKey:key]];
