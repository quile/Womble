...
@implementation IFDBException : CPException
// ?
@end

@implementation IFDBStatement : IFObject
{
...
- executeWithBindValues:(id)bvs {
    if (!bvs) { bvs = [IFArray new]; }
    //[IFLog dump:bvs];
    //[IFLog debug:"SQL " + sql + " BVS " + bvs];
...
@implementation IFDBHandle : IFObject
{
    id dbh; // JDBC handle
